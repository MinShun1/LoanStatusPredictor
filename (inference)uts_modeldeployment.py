...
with st.form("loan_form"):
    person_age = st.slider("Usia", 18, 100, 30)
    person_gender = st.selectbox("Jenis Kelamin", options=["Laki-laki", "Perempuan"])
    person_education = st.selectbox("Tingkat Pendidikan", options=["Tidak diketahui", "SMA", "Associate", "Bachelor", "Master", "Doctorate"])
    person_income = st.number_input("Pendapatan Bulanan", min_value=0, value=3000)
    person_emp_exp = st.slider("Lama Bekerja (tahun)", 0, 85, 5)
    loan_amnt = st.number_input("Jumlah Pinjaman", min_value=100, value=10000)
    loan_int_rate = st.slider("Bunga Pinjaman (%)", 0, 25.0, 10.0)
...
if submitted:
    ...
    education_code = {"Tidak diketahui": 0, "SMA": 0, "Associate": 1, "Bachelor": 2, "Master": 3, "Doctorate": 4}[person_education]
